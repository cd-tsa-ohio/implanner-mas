...
public class ProcessNetworkDealerAgent extends MfgAgent {
	
	Markets m = null; 
	AID returnImpAgent;
	public ProcessNetworkDealerAgent() throws FIPAException {
		super();
	...
	private void incomingQueryMessageHandler(ACLMessage message) {
		...
		if(ire.getTypeName().equals(SLVocabulary.IOTA)){
			...
			try {
				...
				MfgPartModel partModel = (MfgPartModel) ontology.toObject(concept);
				ServiceAction s = new ServiceAction();
				s.setInputObject(partModel);
