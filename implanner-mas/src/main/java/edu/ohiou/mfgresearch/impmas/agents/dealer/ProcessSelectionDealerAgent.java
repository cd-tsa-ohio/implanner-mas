...
public class ProcessSelectionDealerAgent extends MfgAgent {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4184321602665464366L;

	Markets m = null; 
	AID returnImpAgent;
	public ProcessSelectionDealerAgent() throws FIPAException {
		super();
	...
	private void incomingQueryMessageHandler(ACLMessage message) {
		...
		if(ire.getTypeName().equals(SLVocabulary.IOTA)){
			...
			try {
				...
				FactFile factFile = (FactFile) ontology.toObject(concept);
				...
				//Search all vendors 
				DFAgentDescription[] vendors = MfgDF.getInstance().searchByService(this, "Process Selection Member Service", IMPlannerProperties.AgentType.Vendor.toString(), 0);
